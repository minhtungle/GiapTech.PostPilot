
//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated from a template.
//
//     Manual changes to this file may cause unexpected behavior in your application.
//     Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------


namespace EDM_DB
{

using System;
    using System.Collections.Generic;
    
public partial class tbSanPham
{

    public int Stt { get; set; }

    public System.Guid IdSanPham { get; set; }

    public Nullable<System.Guid> IdLoaiSanPham { get; set; }

    public Nullable<System.Guid> IdLoaiKhoaHoc { get; set; }

    public string MaSanPham { get; set; }

    public string TenSanPham { get; set; }

    public Nullable<int> GiaTien { get; set; }

    public Nullable<int> GiaTienTungBuoi { get; set; }

    public Nullable<int> ThoiGianBuoiHoc { get; set; }

    public Nullable<int> SoBuoi { get; set; }

    public string GhiChu { get; set; }

    public Nullable<int> TrangThai { get; set; }

    public Nullable<System.Guid> MaDonViSuDung { get; set; }

    public Nullable<System.DateTime> NgayTao { get; set; }

    public Nullable<System.Guid> IdNguoiTao { get; set; }

    public Nullable<System.DateTime> NgaySua { get; set; }

    public Nullable<System.Guid> IdNguoiSua { get; set; }

}

}
